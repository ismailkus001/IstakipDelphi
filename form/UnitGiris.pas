...
begin
  Application.MessageBox('Kullanici adýný boþ býrakamazsýnýz','HATA',MB_OK+MB_ICONERROR);
...
begin
  Application.MessageBox('Kullanici adýný bulunmadý ','HATA',MB_OK+MB_ICONERROR);
  cxKullaniciAdi.Text:='';
   cxSifre.Text:='';
  cxKullaniciAdi.SetFocus;
  exit;
...
begin
  Application.MessageBox('ÞÝFRE BÖLÜMÜNÜ BOÞ BIRAKMAYIN','HATA',MB_OK+MB_ICONERROR);
  cxSifre.SetFocus;
  exit;
end;
if DM.QGenel.FieldValues['KullaniciSifre']<>cxSifre.Text then
begin
   Application.MessageBox('ÞÝFRE YANLIÞ','HATA',MB_OK+MB_ICONERROR);
   cxSifre.Text:='';
   exit;
end;
try
